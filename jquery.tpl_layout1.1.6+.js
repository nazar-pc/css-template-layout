 ...
 * Modified for multiple usage by Nazar Mokrynskyi <nazar@mokrynskyi.com>
 */
...
(function ($) {
	...
	function getTemplateData (rules) {
		...
		tmp_templates.sort(function (a, b) {
			return !b.slotEl;
		});
	...
	function hasNativeSupport () {
		...
		supported = supported && (test_el.css('display') == test_val);
		test_el.remove();
		return supported;
	}
	...
	function Template (tpl, tpl_sel) {
		...
		function getHeight (slot) {
			var total_h = 0;
			for (var i = 0; i <= slot.row; i++) {
				if (heights[i][slot.col] > 0) {
					total_h += heights[i][slot.col];
					return total_h;
				}
			}
...
// 			if(tpl.slotEl && slots['@']) {
			...
			$.each(slots, function (i, slot) {
				$.each(slots, function (j, s) {
					if (heights[slot.row] === undefined) {
						heights[slot.row] = {};
					}
					if (s.col == slot.col) {
						if (heights[slot.row][slot.col] === undefined) {
							heights[slot.row][slot.col] = 0;
						}
						if (s.row < slot.row) {
							heights[slot.row][slot.col] += s.el.outerHeight(true);
						}
					}
				});
			});
			$.each(rows, function (row_num, row) {
				var cur_row = -1;
				$.each(slots, function (chr, slot) {
					if (max_height[slot.col] === undefined) {
						max_height[slot.col] = 0;
					}
					if (cur_row != row_num) {
						cur_row = row_num;
						i = 0;
					...
					if (slot.row == row_num) { //  && slot.slot_h == 1
						...
						max_height[slot.col] += slot.el.outerHeight();
					}
				});
			...
			for (var i = 0; i < max_height.length; i++) {
				tpl_height = Math.max(tpl_height, max_height[i]);
			}
			if (flex_rows) {
				...
				tpl_el.height(tpl_height + parseInt(tpl_el.css('padding-bottom')));
			}
		}
			...
			if (tot_width < 0) {
				$.each(given_widths, function (i, w) {
					if (w.indexOf('minmax') != -1) {
						...
						var min = minmax[1];
						//var max = minmax[2];
						var min_pixels = getPixelData('w', min, i);
						widths[i] += tot_width;
						tot_width = 0;
		...
		function getTop (slot, offset) {
			var total_h = offset || parseInt(slot.el.parent().css('padding-top').replace('px', ''));
